...
def register_rapor_routes(app):
    ...
    def depo_raporlar():
        ...
        personeller = (
            Kullanici.query.filter_by(rol="kat_sorumlusu", aktif=True)
        ...
        if rapor_tipi:
            if rapor_tipi == "stok_durum":
                ...
                for urun in urunler_liste:
                    ...
                    rapor_verisi.append({"urun": urun, "mevcut_stok": mevcut_stok})
                ...
                if bitis_tarihi:
                    bitis = datetime.strptime(bitis_tarihi, "%Y-%m-%d") + timedelta(
                        days=1
                    )
                ...
                rapor_verisi = (
                    query.order_by(StokHareket.islem_tarihi.desc()).limit(5000).all()
                )

                # Her hareket için zimmet bilgisini ekleyelim
                import re

                _zimmet_id_pattern = re.compile(r"#(\d+)")
                for hareket in rapor_verisi:
                    ...
                    if hareket.aciklama and "Zimmet" in hareket.aciklama:
                        try:
                            match = _zimmet_id_pattern.search(hareket.aciklama)
                            if match:
                                zimmet_id = int(match.group(1))
                                zimmet = db.session.get(PersonelZimmet, zimmet_id)
                                if zimmet and zimmet.personel:
                                    hareket.zimmet_personel = (
                                        f"{zimmet.personel.ad} {zimmet.personel.soyad}"
                                    )
                ...
                rapor_verisi = (
                    query.order_by(PersonelZimmet.zimmet_tarihi.desc())
                    .limit(5000)
                    .all()
                )
                ...
                query = (
                    db.session.query(
                        PersonelZimmetDetay, PersonelZimmet, Kullanici, Urun
                    )
                    .join(
                        PersonelZimmet,
                ...
                query = (
                    ...
                    .filter(
                        ...
                        MinibarIslem.islem_tipi.in_(
                            ["setup_kontrol", "ekstra_tuketim"]
                        ),
                ...
                rapor_verisi = (
                    query.order_by(MinibarIslem.islem_tarihi.desc()).limit(5000).all()
                )
                ...
                query = (
                    db.session.query(
                        ...
                        Kullanici.ad,
                        Kullanici.soyad,
                        func.count(distinct(MinibarIslemDetay.urun_id)).label(
                            "urun_sayisi"
                        ),
                        func.sum(MinibarIslemDetay.tuketim).label("toplam_tuketim"),
                    )
                    .select_from(MinibarIslem)
                    .join(Oda, MinibarIslem.oda_id == Oda.id)
                    .join(Kat, Oda.kat_id == Kat.id)
                    .join(Kullanici, MinibarIslem.personel_id == Kullanici.id)
                    .join(
                    ...
                    .filter(
                        MinibarIslem.islem_tipi.in_(
                            ["setup_kontrol", "ekstra_ekleme", "ekstra_tuketim"]
                        )
                    )
                )
                ...
                query = query.group_by(
                    Oda.oda_no, Kat.kat_adi, Kullanici.ad, Kullanici.soyad
                )
                ...
                ay_baslangic = get_kktc_now().replace(
...
def doluluk_raporu_olustur():
    ...
    try:
        ...
        baslangic_tarih = datetime.strptime(baslangic, "%Y-%m-%d").date()  # pyright: ignore[reportArgumentType]
        bitis_tarih = datetime.strptime(bitis, "%Y-%m-%d").date()  # pyright: ignore[reportArgumentType]
        ...
        while current_date <= bitis_tarih:
            ...
            doluluk_orani = round(
                (rapor["dolu_oda"] / rapor["toplam_oda"] * 100)
                if rapor["toplam_oda"] > 0
                else 0,
                1,
            )
        ...
        ortalama_doluluk = (
            round(toplam_doluluk / gun_sayisi, 1) if gun_sayisi > 0 else 0
        )
        ...
        return render_template(
            "raporlar/doluluk_raporlari.html",
            oteller=oteller,
            rapor_verisi=rapor_verisi,
            secili_otel_id=otel_id,
            secili_baslangic=baslangic,
...
def zimmet_raporlari():
    ...
    try:
        ...
        return render_template(
            "raporlar/zimmet_raporlari.html", oteller=oteller, personeller=personeller
        )
    except Exception as e:
...
def performans_raporlari():
    ...
    try:
        ...
        return render_template(
            "raporlar/performans_raporlari.html",
            oteller=oteller,
            personeller=personeller,
        )
    except Exception as e:
...
def doluluk_excel():
    ...
    try:
        ...
        ws["A2"] = (
            f"{baslangic_tarih.strftime('%d.%m.%Y')} - {bitis_tarih.strftime('%d.%m.%Y')}"
        )
...
def mevcut_stok_raporu_olustur():
    ...
    try:
        ...
        urunler_data = (
            ...
            .order_by(Otel.ad, Urun.urun_adi)
            .all()
        )
...
def stok_hareketleri_raporu_olustur():
    ...
    try:
        ...
        baslangic_tarih = datetime.strptime(baslangic, "%Y-%m-%d")  # pyright: ignore[reportArgumentType]
        bitis_tarih = datetime.strptime(bitis, "%Y-%m-%d")  # pyright: ignore[reportArgumentType]
...
def stok_excel():
    ...
    try:
        ...
        if tip == "mevcut":
            ...
            urunler = (
                query.group_by(Otel.ad, Urun.urun_adi, Urun.barkod, Urun.birim)
                .order_by(Otel.ad, Urun.urun_adi)
                .all()
            )
            ...
            hareketler = (
                query.order_by(StokHareket.islem_tarihi.desc()).limit(10000).all()
            )
            ...
            for hareket in hareketler:
                ws.cell(
                    row=row,
                    column=1,
                    value=hareket.islem_tarihi.strftime("%d.%m.%Y %H:%M"),
                )
                ...
                ws.cell(
                    row=row,
                    column=7,
                    value=f"{hareket.kullanici_ad} {hareket.kullanici_soyad}",
                )
                row += 1
...
def minibar_detayli_raporu_olustur():
    ...
    try:
        ...
        islemler_data = query.limit(5000).all()
        ...
        log_islem(
            "view",
            "minibar_detayli_raporu",
            {"otel_id": otel_id, "baslangic": baslangic, "bitis": bitis},
        )

        return render_template(
            "raporlar/minibar_raporlari.html",
            oteller=oteller,
...
def minibar_ozet_raporu_olustur():
    ...
    try:
        ...
        for u in urunler_data:
            urunler.append(
                {"urun": u.urun_ad, "miktar": u.toplam_miktar, "birim": u.birim}
            )
        ...
        return render_template(
            "raporlar/minibar_raporlari.html", oteller=oteller, ozet_rapor=rapor_verisi
        )
...
def minibar_excel():
    try:
        ...
        if tip == "detayli":
            ...
            islemler = query.limit(10000).all()
            ...
            for islem in islemler:
                ws.cell(
                    row=row,
                    column=1,
                    value=islem.islem_tarihi.strftime("%d.%m.%Y %H:%M"),
                )
                ...
                ws.cell(
                    row=row,
                    column=5,
                    value=f"{islem.personel_ad} {islem.personel_soyad}",
                )
                row += 1
...
def zimmet_raporu_olustur():
    ...
    try:
        ...
        query = (
            ...
            .join(
                PersonelZimmetDetay, PersonelZimmet.id == PersonelZimmetDetay.zimmet_id
            )
            .join(Kullanici, PersonelZimmet.personel_id == Kullanici.id)
            .join(Urun, PersonelZimmetDetay.urun_id == Urun.id)
            .join(Otel, Kullanici.otel_id == Otel.id)
        ...
        zimmetler_data = (
            query.order_by(PersonelZimmet.zimmet_tarihi.desc()).limit(5000).all()
        )
            ...
            zimmetler.append(
                {
                    ...
                    "iade_tarihi": z.iade_tarihi.strftime("%d.%m.%Y")
                    if z.iade_tarihi
                    else None,
                    "durum": z.durum,
                }
        ...
        return render_template(
            "raporlar/zimmet_raporlari.html",
            oteller=oteller,
            personeller=personeller,
            rapor_verisi=rapor_verisi,
        )
...
def zimmet_excel():
    ...
    try:
        ...
        zimmetler = (
            query.order_by(PersonelZimmet.zimmet_tarihi.desc()).limit(10000).all()
        )

        row = 5
        for zimmet in zimmetler:
            ws.cell(
                row=row, column=1, value=f"{zimmet.personel_ad} {zimmet.personel_soyad}"
            )
            ...
            ws.cell(
                row=row,
                column=6,
                value=zimmet.iade_tarihi.strftime("%d.%m.%Y")
                if zimmet.iade_tarihi
                else "-",
            )
        ...
        log_islem(
            "export", "zimmet_raporu", {"personel_id": personel_id, "durum": durum}
        )
...
def performans_excel():
    ...
    try:
        ...
        for perf in performanslar:
            ws.cell(
                row=row, column=1, value=f"{perf.personel_ad} {perf.personel_soyad}"
            )
...
def otel_zimmet_stok_raporu_olustur():
    ...
    try:
        ...
        rapor = OtelZimmetRaporServisi.get_otel_zimmet_stok_raporu(otel_id)  # pyright: ignore[reportArgumentType]
...
def export_otel_zimmet_stok_pdf(rapor):
    ...
    for idx, otel in enumerate(rapor.get('oteller', [])):
        ...
        for urun in otel.get('urunler', []):
            ...
            table_data.append(
                [
                    Paragraph(
                        urun["urun_adi"],
                        ParagraphStyle("Cell", fontName=base_font, fontSize=8),
        ...
        kritik_urun = sum(
            1
            for u in otel.get("urunler", [])
            if u["stok_durumu"] in ["stokout", "kritik"]
        )
        elements.append(
            Paragraph(
                f"Toplam {toplam_urun} ürün | {kritik_urun} ürün kritik/tükendi durumunda",
                ozet_style,
            )
        )
        ...
        return jsonify(
            {
                "success": True,
                "personeller": [
                    {"id": p.id, "ad_soyad": f"{p.ad} {p.soyad}"} for p in personeller
                ],
            }
        )
    except Exception as e:
...
def kat_sorumlusu_kullanim_raporu_olustur():
    ...
    try:
        ...
        baslangic_tarihi = (
            datetime.strptime(baslangic_str, "%Y-%m-%d").date()
            if baslangic_str
            else date.today()
        )
        bitis_tarihi = (
            datetime.strptime(bitis_str, "%Y-%m-%d").date()
            if bitis_str
            else date.today()
        )
        ...
        rapor = KatSorumlusuGunSonuRaporServisi.get_gun_sonu_raporu(
            otel_id=otel_id,
            personel_ids=personel_ids if personel_ids else None,  # pyright: ignore[reportArgumentType]
        ...
        log_islem(
            "view",
            "kat_sorumlusu_gun_sonu_raporu",
            {"otel_id": otel_id, "baslangic": baslangic_str, "bitis": bitis_str},
        )

        return render_template(
            "raporlar/kat_sorumlusu_kullanim.html",
            oteller=oteller,
            rapor_verisi=rapor,
            secili_otel_id=otel_id,
...
def kat_sorumlusu_kullanim_excel():
    ...
    try:
        ...
        personel_ids = (
            [int(x) for x in personel_ids_str.split(",") if x.strip().isdigit()]
            if personel_ids_str
            else None
        )
        baslangic_tarihi = (
        ...
        rapor = KatSorumlusuGunSonuRaporServisi.get_gun_sonu_raporu(
            otel_id=otel_id,
            personel_ids=personel_ids,  # pyright: ignore[reportArgumentType]
            baslangic_tarihi=baslangic_tarihi,
            bitis_tarihi=bitis_tarihi,
        )
...
def export_gun_sonu_pdf(rapor):
    ...
    header_table = Table(
        [
            [
                logo_cell,
                Paragraph(center_content, styles["Normal"]),
                Paragraph(date_content, styles["Normal"]),
            ]
        ],
...
def oda_bazli_tuketim_raporu_olustur():
    ...
    try:
        ...
        baslangic_tarihi = (
            datetime.strptime(baslangic, "%Y-%m-%d").date() if baslangic else None
        )
        ...
        rapor = OdaBazliTuketimRaporServisi.get_oda_bazli_tuketim_raporu(
            otel_id=otel_id,
            baslangic_tarihi=baslangic_tarihi,  # pyright: ignore[reportArgumentType]
            bitis_tarihi=bitis_tarihi,  # pyright: ignore[reportArgumentType]
        )
        ...
        log_islem(
            "view",
            "oda_bazli_tuketim_raporu",
            {"otel_id": otel_id, "baslangic": baslangic, "bitis": bitis},
        )

        return render_template(
            "raporlar/oda_bazli_tuketim.html", oteller=oteller, rapor_verisi=rapor
        )
...
def gunluk_gorev_detay_raporlari():
    ...
    try:
        ...
        return render_template(
            "raporlar/gunluk_gorev_detay.html",
            oteller=oteller,
            secili_otel_id=otel_id,
...
def gunluk_gorev_detay_raporu_olustur():
    ...
    try:
        ...
        rapor = GunlukGorevRaporServisi.get_gunluk_gorev_raporu(
            otel_id=otel_id,  # pyright: ignore[reportArgumentType]
            personel_ids=personel_ids,  # pyright: ignore[reportArgumentType]
            baslangic_tarihi=tarih_obj,  # pyright: ignore[reportArgumentType]
            bitis_tarihi=tarih_obj,  # pyright: ignore[reportArgumentType]
        )
...
def kat_sorumlusu_gun_sonu_raporum():
    ...
    try:
        ...
        otel = db.session.get(Otel, kullanici.otel_id)
        ...
        return render_template(
            "raporlar/kat_sorumlusu_gun_sonu_raporum.html",
            otel=otel,
            personeller=personeller,
