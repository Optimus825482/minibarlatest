...
def register_depo_routes(app):
    ...
    def stok_giris():
        ...
        if request.method == 'POST':
            try:
                ...
                if miktar <= 0 or miktar > 10000:
                    flash("Miktar 1 ile 10.000 arasında olmalıdır.", "warning")
        ...
        return render_template(
            "depo_sorumlusu/stok_giris.html",
            gruplar=gruplar,
            urunler=urunler,
    ...
    def stok_duzenle(hareket_id):
        ...
        if request.method == "POST":
            try:
                ...
                if yeni_miktar <= 0 or yeni_miktar > 10000:
                    flash("Miktar 1 ile 10.000 arasında olmalıdır.", "warning")
        ...
        urunler = Urun.query.filter_by(aktif=True).order_by(Urun.urun_adi).all()
        return render_template(
            "depo_sorumlusu/stok_duzenle.html", hareket=hareket, urunler=urunler
        )
    ...
    def personel_zimmet():
        ...
        if request.method == "POST":
            try:
                ...
                fifo_stoklar = FifoStokServisi.toplu_stok_getir(
                    otel_id, list(urun_miktarlari.keys())
                )
                urun_kayitlari = {
                    urun.id: urun
                    for urun in Urun.query.filter(
                        Urun.id.in_(urun_miktarlari.keys())
                    ).all()
                }
                ...
                for uid, talep_miktari in urun_miktarlari.items():
                    ...
                    if talep_miktari > mevcut:
                        ...
                        urun_adi = (
                            f"{urun.urun_adi} ({urun.birim})" if urun else f"ID {uid}"
                        )
                        yetersiz_stok.append((urun_adi, talep_miktari, mevcut))

                if yetersiz_stok:
                    detay_mesaji = "; ".join(
                        f"{urun_adi}: istenen {talep}, mevcut {mevcut}"
                        for urun_adi, talep, mevcut in yetersiz_stok
                    )
                ...
                for uid, miktar in urun_miktarlari.items():
                    ...
                    if not fifo_sonuc["success"]:
                        raise Exception(
                            f"FIFO stok çıkışı hatası: {fifo_sonuc['message']}"
                        )
        ...
        if secili_otel_id:
            kat_sorumlulari = Kullanici.query.filter_by(
                rol="kat_sorumlusu", aktif=True, otel_id=secili_otel_id
            ).all()
        else:
            kat_sorumlulari = Kullanici.query.filter_by(
                rol="kat_sorumlusu", aktif=True
            ).all()
        ...
        aktif_zimmetler = (
            PersonelZimmet.query.options(
                ...
                joinedload(PersonelZimmet.detaylar).joinedload(
                    PersonelZimmetDetay.urun
                ),
            )
            .filter(PersonelZimmet.durum.in_(["aktif", "iade_edildi"]))
        ...
        return render_template(
            "depo_sorumlusu/personel_zimmet.html",
        ...
        try:
            ...
            query = (
                KatSorumlusuSiparisTalebi.query.options(
                    ...
                    joinedload(KatSorumlusuSiparisTalebi.kat_sorumlusu).joinedload(
                        Kullanici.otel
                    ),
                )
                .join(
                    Kullanici,
                    KatSorumlusuSiparisTalebi.kat_sorumlusu_id == Kullanici.id,
                )
                .filter(KatSorumlusuSiparisTalebi.durum == "beklemede")
            )
            ...
            siparisler = (
                query.order_by(KatSorumlusuSiparisTalebi.talep_tarihi.desc())
            ...
            for siparis in siparisler:
                kat_sorumlusu = (
                    siparis.kat_sorumlusu if hasattr(siparis, "kat_sorumlusu") else None
                )
                otel_adi = (
                    kat_sorumlusu.otel.ad
                    if kat_sorumlusu and kat_sorumlusu.otel
                    else "Bilinmeyen"
                )
                ...
                siparis_listesi.append(
                    {
                        ...
                        "personel": f"{kat_sorumlusu.ad} {kat_sorumlusu.soyad}"
                        if kat_sorumlusu
                        else "Bilinmeyen",
                        "otel": otel_adi,
            ...
            return jsonify({"success": True, "siparisler": siparis_listesi})
    ...
    def depo_stoklarim():
        ...
        try:
            ...
            if secili_otel_id:
                ...
                otel_stok_map = {
                    stok.urun_id: stok.mevcut_stok for stok in otel_stoklar
                }
            else:
                toplam_stoklar = (
                    db.session.query(
                        UrunStok.urun_id, func.sum(UrunStok.mevcut_stok).label("toplam")
                    )
                    .filter(UrunStok.otel_id.in_(otel_ids))
                    .group_by(UrunStok.urun_id)
                    .all()
                )
            ...
            for urun in urunler:
                ...
                kritik_seviye = (
                    urun.kritik_stok_seviyesi or DEFAULT_KRITIK_STOK_SEVIYESI
                )
                durum = "yeterli"
                if mevcut_stok == 0:
                    durum = "tukendi"
        ...
        try:
            ...
            query = (
                KatSorumlusuSiparisTalebi.query.options(
                    ...
                    joinedload(KatSorumlusuSiparisTalebi.detaylar).joinedload(
                        KatSorumlusuSiparisTalepDetay.urun
                    ),
                )
                .join(
                    Kullanici,
            ...
            return render_template(
                "depo_sorumlusu/kat_sorumlusu_siparisler.html", siparisler=siparisler
            )
    ...
    def api_depo_siparis_kabul(siparis_id):
        ...
        try:
            ...
            if zimmet.durum != "aktif":
                return jsonify(
                    {"success": False, "error": "Bu sipariş zaten işlenmiş"}
                ), 400
            ...
            for detay in detaylar:
                hareket = (
                    StokHareket.query.filter_by(
                        urun_id=detay.urun_id,
                        aciklama=f"Zimmet atama - {zimmet.personel.ad} {zimmet.personel.soyad}",
                    )
                    .order_by(desc(StokHareket.id))
                    .first()
                )
                if hareket:
            ...
            return jsonify(
                {"success": False, "error": "Sipariş kabul sırasında bir hata oluştu"}
            ), 500
    ...
    def api_depo_siparis_iptal(siparis_id):
        ...
        try:
            ...
            if zimmet.teslim_eden_id is not None:
                return jsonify(
                    {"success": False, "error": "Bu bir zimmet kaydıdır, sipariş değil"}
                ), 400
            ...
            return jsonify(
                {"success": True, "message": "Sipariş başarıyla iptal edildi."}
            )
            ...
            return jsonify(
                {"success": False, "error": "Sipariş iptal sırasında bir hata oluştu"}
            ), 500

    @app.route(
        "/api/depo/kat-sorumlusu-siparis-reddet/<int:talep_id>", methods=["POST"]
    )
    @login_required
    @role_required("depo_sorumlusu")
    def api_kat_sorumlusu_siparis_reddet(talep_id):
        ...
        try:
            ...
            if not talep:
                return jsonify(
                    {"success": False, "message": "Sipariş talebi bulunamadı"}
                ), 404
            ...
            return jsonify(
                {"success": True, "message": "Sipariş talebi başarıyla reddedildi"}
            )
            ...
            return jsonify(
                {"success": False, "message": "Red işlemi sırasında bir hata oluştu"}
            ), 500

    @app.route(
        "/api/depo/kat-sorumlusu-siparis-onayla/<int:talep_id>", methods=["POST"]
    )
    @login_required
    @role_required("depo_sorumlusu")
    def api_kat_sorumlusu_siparis_onayla(talep_id):
        ...
        try:
            ...
            return jsonify(
                {"success": False, "message": "Onay işlemi sırasında bir hata oluştu"}
            ), 500
    ...
    def manuel_gorev_raporu_gonder():
        ...
        try:
            ...
            if kat_sorumlusu_id:
                ...
                if (
                    ks
                    and ks.rol == "kat_sorumlusu"
                    and (
                        session.get("rol") == "sistem_yoneticisi"
                        or ks.otel_id in otel_ids
                    )
                ):
                    ...
                    if result.get("success"):
                        gonderilen += 1
                        sonuclar.append(
                            {"ad": f"{ks.ad} {ks.soyad}", "durum": "başarılı"}
                        )
                    else:
                        hatali += 1
                        sonuclar.append(
            ...
            return jsonify(
                {
                    "success": False,
                    "message": "Rapor gönderimi sırasında bir hata oluştu",
                }
            ), 500
    ...
    def manuel_minibar_raporu_gonder():
        ...
        try:
            ...
            if otel_id:
                ...
                if otel and (
                    session.get("rol") == "sistem_yoneticisi" or otel.id in otel_ids
                ):
                    result = RaporEmailService.send_minibar_raporu(
                        otel.id, rapor_tarihi
                    )
                    if result.get("success"):
                        gonderilen += 1
                        ...
                        hatali += 1
                        sonuclar.append(
                            {"ad": otel.ad, "durum": result.get("message", "hata")}
                        )
            else:
                if session.get("rol") == "sistem_yoneticisi":
            ...
            return jsonify(
                {
                    "success": False,
                    "message": "Minibar raporu gönderimi sırasında bir hata oluştu",
                }
            ), 500
    ...
    def ana_depo_tedarik():
        ...
        try:
            ...
            query = AnaDepoTedarik.query.options(
                ...
                joinedload(AnaDepoTedarik.detaylar).joinedload(
                    AnaDepoTedarikDetay.urun
                ),
            )
            ...
            if not tum_islemler:
                ...
                ay_sonu = datetime(
                    secili_yil, secili_ay, son_gun, 23, 59, 59, tzinfo=KKTC_TZ
                )
                query = query.filter(AnaDepoTedarik.islem_tarihi >= ay_basi)
                query = query.filter(AnaDepoTedarik.islem_tarihi <= ay_sonu)

            tedarikler = (
                query.order_by(desc(AnaDepoTedarik.islem_tarihi)).limit(2000).all()
            )
                    ...
                    try:
                        ...
                        if 'duplicate key' in exc_msg or 'unique' in exc_msg:
                            ...
                            if retry == max_retry - 1:
                                raise Exception(
                                    "Tedarik numarası oluşturulamadı. Lütfen tekrar deneyin."
                                )
                            continue
                        else:
                            raise
                            ...
                            if zimmet and zimmet.durum == 'aktif':
                                if zimmet_detay.kullanilan_miktar > 0:
                                    return jsonify(
                                        {
                                            "success": False,
                                            "error": "Tedarik edilen ürünler minibar işlemlerinde kullanılmış. İptal edilemez.",
                                        }
                                    ), 400
